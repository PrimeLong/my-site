...
import {
  ...
  accountRegister, accountLogin, accountMe, accountUpdate, accountPassword, accountLogout,
} from './lib/client.js';
import {
  ...
  Star, Shield, Anchor, User, LogIn, LogOut, KeyRound,
} from 'lucide-react';
import {
...
/* Вход и регистрация. reason — зачем просим войти (например, перед сетевой игрой). */
  ...
  const submit = async (e) => {
    ...
    try {
      const r = tab === 'register'
        ? await accountRegister(login.trim().toLowerCase(), password, name.trim(), getPlayerId())
        : await accountLogin(login.trim().toLowerCase(), password);
      if (!r || !r.token || !r.profile) throw new Error('Сервер не ответил');
      const playerId = adoptProfile(r.token, r.profile);
      Audio.play('up');
      if (onDone) onDone(r.profile, playerId);
      onClose();
    } catch (err) { setError(err.message); Audio.play('down'); } finally { setBusy(false); }
  };
  return (
    <ModalShell title={tab === 'register' ? 'Регистрация' : 'Вход в профиль'} label="Профиль игрока" icon={User} onClose={onClose}>
      ...
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <label style={{ fontSize: 12 }}>Пароль
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
            autoComplete={tab === 'register' ? 'new-password' : 'current-password'} placeholder="не короче 6 символов"
        ...
        <button type="submit" className="ems-btn primary" disabled={busy || login.trim().length < 3 || password.length < 6}
          style={{ padding: '11px 0', marginTop: 2 }}>
          {busy ? 'Минутку…' : tab === 'register' ? 'Создать профиль' : 'Войти'}
        </button>
      </form>
    </ModalShell>
  );
...
/* Профиль: значок, имя, статистика сетевых партий, пароль и выход. */
function ProfileModal({ onClose, onSwitched, onLink }) {
  ...
  React.useEffect(() => {
    ...
    accountMe(account.token).then((r) => {
      ...
      setProfile(r.profile); setName(r.profile.name);
  ...
  const changePw = async () => {
    setBusy(true); setError(''); setNote('');
    try { await accountPassword(account.token, oldPw, newPw); setOldPw(''); setNewPw(''); setPwOpen(false); setNote('Пароль изменён'); }
    catch (e) { setError(e.message); } finally { setBusy(false); }
  };
  const logout = async () => {
  ...
  return (
    <ModalShell title="Профиль" icon={User} onClose={onClose}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
        {!pwOpen && <button className="ems-btn" style={{ padding: '6px 10px', fontSize: 12 }} onClick={() => setPwOpen(true)}><KeyRound size={12} style={{ verticalAlign: -2, marginRight: 5 }} />Пароль</button>}
...
function ProfileChip({ account, onClick }) {
