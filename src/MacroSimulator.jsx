...
const ROLES = [
  ...
  { id: 'full_control', icon: 'globe', title: 'Глава государства', short: 'Полное управление',
...
function botCentralBank(s, personaId, difficulty) {
  ...
  return {
    ...
    note: `Центральный банк (${P.name}) ${parts.join(', ')}.`, demand, stance, institution: 'cb',
...
function botFinanceMinistry(s, personaId, difficulty) {
  ...
  return {
    ...
    note: `Минфин (${P.name}) ${parts.join(', ')}.`, demand, stance, institution: 'gov',
    detail: [
...
function simulateQuarter({ economy, decisions, pendingImpulses, eventCooldowns, difficulty, quarterIndex, stories, botAction, botActions, publicMode }) {
  ...
  const transfersGrowth = clamp(trendReal + decisions.transfers + (d.transfersPressure || 0), -12, 16);
  const govInvestmentGrowth = clamp(trendReal + decisions.govInvestment, -16, 20);
  ...
  const capitalIncomeGrowth = s.gdpGrowth - s.inflation * 0 + 0.5 * (s.gdpGrowth - s.potentialGrowth);
  ...
  const bop = currentAccountPrev + netCapitalFlow + decisions.fxIntervention * 0;
  ...
  const adaptSpeed = clamp(C.expAdaptMin + (C.expAdaptMax - C.expAdaptMin) * clamp(inflDev / 6, 0, 1) * (1 - s.cbCredibility / 140), 0.08, 0.55);
  const anchorWeight = clamp(s.cbCredibility / 100 * (1 - clamp((inflDev - 2) / 8, 0, 0.6)), 0.05, 0.95);
  ...
  const cbCredibility = clamp(s.cbCredibility + credDrift * 0.55 - Math.abs(targetChange) * 9 + gauss(0.5 * nMult), 0, 100);
  ...
  const businessConfidence = clamp(s.businessConfidence + 0.11 * (T.neutralConfidence - s.businessConfidence)
    + 0.32 * (gdpGrowth - potentialGrowth) - 0.06 * Math.max(0, bankingRisk - 50) - 0.25 * Math.max(0, rateGap)
  ...
  const approval = clamp(ema(Number.isFinite(s.approval) ? s.approval : 55, approvalTarget, 0.28), 0, 100);
...
function generateNews(prev, s, decisions, quarterIndex, botAction, cd, extraActions, publicMode) {
  ...
  let picked = VOICES.filter((v) => v.when()).sort((a, b) => b.p - a.p).filter((v) => once(`op:${v.id}`, 7)).slice(0, 2);
  if (picked.length === 0) {
    ...
    if (changedCourse || newDemand || once(`bot${ix}`, 8)) {
      if (publicMode) {
        ...
        push(isCb ? 'cb' : 'gov', act.newsHeadline,
          `${act.note}${act.quote ? ` Из заявления по итогам решения: «${act.quote}»` : ''}${act.demand ? ` ${act.demand}` : ''}`,
...
const Audio = (() => {
  ...
  const ensure = () => {
    ...
    } catch (e) { verbIn = ctx.createGain(); verbIn.gain.value = 0; verbIn.connect(musicBus); }
  ...
  const panFor = (midi, width) => {
    try { const p = ctx.createStereoPanner(); p.pan.value = clamp((midi - 62) / 30, -1, 1) * (width || 0.35); return p; } catch (e) { return ctx.createGain(); }
  };
  ...
  return {
    ...
    play(name) {
      ...
      const fn = SFX[name]; if (fn) { try { fn(); } catch (e) { /* тишина важнее падения */ } }
    },
    startMusic() {
      if (!ensure()) return; resume();
      ...
      timer = setInterval(() => { try { scheduler(); } catch (e) { /* музыка не ломает игру */ } }, 40);
      notify();
    },
    stopMusic() {
      running = false;
    ...
    setAmbience(regime, k) {
      ...
      if (crisis && k > 0.5) {
        if (!heartTimer) heartTimer = setInterval(() => { try { heartbeat(); } catch (err) { /* тихо */ } }, 1700);
...
function SaveLoadModal({ mode, snapshot, onClose, onLoad }) {
  ...
  const download = () => {
    ...
    } catch (e) { setError('Скачивание недоступно в этом окружении — скопируйте текст вручную.'); }
  };
  const copy = () => {
    try {
    ...
    } catch (e) { setError('Не удалось скопировать автоматически — выделите текст вручную.'); }
  };
  const readFile = (e) => {
...
const haptic = (pattern) => { try { if (typeof navigator !== 'undefined' && navigator.vibrate) navigator.vibrate(pattern); } catch (e) { /* не поддерживается */ } };
