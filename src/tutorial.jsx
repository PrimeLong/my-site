      ...
      },
      {
        ...
        body: () => (
          <>
            ...
            <p>Если президент всё же начинает военную операцию, воевать приходится на карте. В Норланде три цели: Ледяной перевал у границы, Копи Хальвика и столица Нордхольм — к ней не подойти, пока перевал не взят. Приказ — цель и способ — действует, пока вы его не смените, а сама война не кончается по расписанию: её заканчивает перемирие или капитуляция Норланда, и чем дольше она идёт, тем сильнее усталость страны. Способы такие: штурм быстро двигает фронт, но стоит денег и потерь; осада медленнее и дешевле; удержание почти не двигает фронт, зато гасит контратаки; перемирие закрывает войну по линии фронта. Сила армии зависит от доли обороны в бюджете и от поддержки в стране. Копи дают экспорт руды, перевал облегчает следующие штурмы, а взятый Нордхольм при остальных целях — капитуляция Норланда. Санкции за войну при этом никуда не деваются. Когда война кончается, взятые земли остаются за вами: граница на карте сдвигается, новые области прибавляют рабочую силу, а копи — экспорт. Следующая операция начинается с того, что осталось у Норланда.</p>
            <p>Новые земли — полноценные области со своей лояльностью. Она стартует низкой, и пока она ниже 35, там партизаны: подрывы на перевале, саботаж на копях, подполье в Нордхольме. Программа интеграции стоит денег и прибавляет около 6 пунктов за квартал; стройка в области и мягкие ответы на её события тоже помогают. С 50 область интегрирована и начинает голосовать.</p>
            <p>Перемирие не решает, чья это земля. После войны открываются переговоры: признание границы, поддержка Норландом снятия санкций, репарации, возврат части земель. У каждого условия своя цена, и Норланд соглашается, если цена не выше позиции страны — а она тает, пока тянут время. Непризнанная граница каждый квартал бьёт по экспорту и инвестициям и кормит реваншизм Норланда. Когда он дойдёт до 100, Норланд нападёт, чтобы вернуть своё: каждый квартал он бьёт по одной из новых областей, разведка сообщает куда, но иногда ошибается, и вы выбираете, какую область укрепить и как — держать оборону, контрудар или переговоры.</p>
          </>
        ),
      },
    ],
...
const TUTORIAL_COURSES = [
  { id: 'policy', icon: Landmark, title: 'Экономическая политика',
    lede: 'Базовый курс: как ставка, бюджет, курс валюты и ожидания связаны между собой. Восемь модулей от поверхностного понимания к углублённому, каждый заканчивается тестом и практической задачей.',
