    ...
    steps: [
      {
        ...
        body: () => (
          <>
            <p>Режим меняется двумя путями. Первый — сам, от накопленного напряжения: демократия → конфликт ветвей власти → авторитаризм → тоталитаризм. Напряжение растёт от низкого рейтинга, кризисов, безработицы и инфляции — и, отдельно, от самих репрессий. Когда оно спадает, лестница идёт и вниз: конфликт ветвей гаснет, чрезвычайное положение снимают.</p>
            <p>Второй — решение президента: распустить парламент указом. Режим становится авторитарным сразу, и спокойная обстановка его уже не отменит — такой режим держится, пока его не отменят. Отменить его может президент (указ «вернуть парламент»; бот-президент с демократическим характером так и делает, когда напряжение спадёт) — или улица и армия: при очень высоком напряжении власть свергают восстание или переворот.</p>
          </>
        ),
      },
      {
    ...
    steps: [
      {
        ...
        body: () => (
          <>
            <p>Войну начинает только президент — на карте, из карточки той страны, которой её объявляют: Норланду, Вестравии или Дешту. У каждой страны три цели операции, и к столице не подойти, пока не взяты подступы:</p>
            <p><b>Норланд</b> — Ледяной перевал у границы, Копи Хальвика и столица Нордхольм (к ней — через перевал или копи). Единственный сосед, чьи взятые земли можно присоединить. <b>Дешт</b> — приграничные степи, Кумсайские нефтепромыслы (взятые — дешевле топливо) и столица Ашкала; армия у него большая. <b>Вестравия</b> — пограничные укрепления, Лиммерн и Вестград; брать её вдвое труднее, а война с главным торговым партнёром каждый квартал бьёт по экспорту и кредиту. У Дешта и Вестравии земли не присоединяются: войну с ними закрывают репарации.</p>
            ...
            <p>Сила армии зависит от доли обороны в бюджете и от поддержки в стране. Война не кончается по расписанию: её заканчивает перемирие или капитуляция противника, которая наступает, когда взяты все три цели. Чем дольше идёт война, тем сильнее устаёт страна, а санкции за нападение никуда не деваются.</p>
          </>
        ),
      },
      {
        ...
        body: () => (
          <>
            <p>Когда война с Норландом кончается, взятые земли остаются за вами: граница на карте сдвигается, новые области прибавляют рабочую силу, а копи — экспорт руды. (С Дештом и Вестравией так не выйдет: взятое у них возвращается по миру в обмен на выплаты.)</p>
    ...
    quiz: {
      ...
      questions: [
        q('Президент распустил парламент указом при спокойной экономике. Что будет дальше?',
          ['Режим станет авторитарным и продержится, пока президент не вернёт парламент (или власть не свергнут)',
          ...
          'Чрезвычайное положение, введённое в кризис, снимается, когда кризис проходит. Осознанный роспуск — другое: спокойная экономика его не отменит. Отменяет его решение президента вернуть парламент — или переворот и восстание, если напряжение зашкалит.'),
