...
const TUTORIAL_MODULES = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: 'Обещания и карта округов',
        lever: null, runsQuarter: false,
        body: () => (
          <>
            ...
            <p>Итог приходит не одной цифрой на всю страну, а картой из семи округов. Доля голосов за действующую власть складывается из двух вещей: структурной склонности округа — столица традиционно голосует против власти, село за неё — и того, как округу живётся сейчас. Округ, которому хуже среднего по стране, отворачивается сильнее, поэтому одна и та же средняя цифра выглядит на карте по-разному, и видно, чем именно недовольны там, где недовольны.</p>
      ...
      {
        title: 'Стройки и события в округах',
        lever: null, runsQuarter: false,
        body: () => (
          <>
            <p>Карта — не только табло. У каждого округа есть своя большая стройка по его характеру: метро в столице, глубоководный порт в Портовске, электростанция в шахтёрских горах, ирригация в Хлебородье, железная дорога на Лесную окраину. Запускает её Минфин, а живой президент — поверх него, прямо на карте.</p>
            <p>Стройка идёт несколько кварталов и всё это время стоит денег — это госинвестиции сверх ползунка, они добавляют спрос, дефицит и инфраструктуру. Пока идёт работа, в округе спокойнее. Достроенная навсегда снижает его напряжение — а значит, и прибавляет голоса там, — и даёт свой эффект: порт — экспорт, электростанция — дешёвую энергию и меньшую инфляцию, модернизация заводов — производительность. Одновременно — не больше трёх строек, и ни одной, пока страна без доступа к рынку.</p>
            <p>Округа сами подбрасывают задачи: забастовка шахтёров, засуха, авария в порту, паника вкладчиков, закрытие завода, митинг у правительства, лесные пожары. Событие вспыхивает на карте, и ответить нужно в следующем квартале. У каждого варианта своя цена: щедрый ответ стоит денег и успокаивает округ, дешёвый экономит бюджет, жёсткий гасит проблему силой и копит напряжение в стране. Промолчать тоже можно — тогда сработает вариант «переждать», и он подписан заранее.</p>
          </>
        ),
      },
      {
